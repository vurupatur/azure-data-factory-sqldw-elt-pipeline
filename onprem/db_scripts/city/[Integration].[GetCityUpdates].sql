EXEC [dbo].[DropProcedureIfExists] 'Integration', 'GetCityUpdates'

PRINT 'Creating procedure [Integration].[GetCityUpdates]'
GO
    ...
    BEGIN
        INSERT #CityChanges
            ([WWI City ID], City, [State Province], Country, Continent, [Sales Territory], Region, Subregion,
             --[Location],
			 [Latest Recorded Population], [Valid From], [Valid To],[Location],[StateProvinceCode])
        SELECT c.CityID, c.CityName, sp.StateProvinceName, co.CountryName, co.Continent, sp.SalesTerritory, co.Region, co.Subregion,
               --c.[Location],
			   COALESCE(c.LatestRecordedPopulation, 0), @ValidFrom, NULL,NULL,sp.StateProvinceCode
